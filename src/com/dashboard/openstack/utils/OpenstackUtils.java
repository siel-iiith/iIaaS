...
import com.dashboard.resources.*;
...
public class OpenstackUtils {
	 ...
	 * @param tenantName - tenantName for openstack account
	 * @returns the connection object on successful authentication
	 */
	public static Connection authenticate(String username, String password, String tenantName){
		...
		cb.endpoint("http://10.2.4.129:5000/v2.0");
		ComputeServiceContext context = cb
	 ...
	 */
	public static Map<Integer, Volume> getVolumes (ComputeServiceContext context) {
		Map<Integer, Volume> volumesMap = new HashMap<Integer, Volume>();
		Optional<? extends VolumeApi> volumeTypeOption;
		NovaApi novaApi = NovaApi.class.cast(context.getProviderSpecificContext().getApi());
	...
	public static void resumeNode(ComputeService compute, String nodeId) {
		try {
			System.out.println("---------------------b4 returned---------------------");
			compute.resumeNode(nodeId);
			
		} catch (Exception e) {
			System.out.println("Exception Suppressed");
			System.out.println("---------------------returned---------------------");
			Map<Integer, Hardware> flvrsMap = Utils.getFlavors("openstack",compute);
			
			//System.out.println("size  "+instancesMap.size());
			boolean flag=true;
			while(flag)
			{
				Map<String, Instance> instancesMap = Utils.getInstances("openstack", compute, flvrsMap);
	 ...
	 * @param nodeId - Id of the node
	 */
	public static void attachVolume (ComputeServiceContext context, String volumeId, String nodeId) {
		
		boolean flag=true;
		System.out.println("attaching  " + volumeId);
		Map<Integer, Volume> volumesMap = new HashMap<Integer, Volume>();
		Optional<? extends VolumeApi> volumeTypeOption;
		NovaApi novaApi = NovaApi.class.cast(context.getProviderSpecificContext().getApi());
		...
		VolumeAttachment result = volumesApi.attachVolumeToServerAsDevice(volumeId, nodeId, "/dev/vdc");
		System.out.println("check 3");
		
		while(flag)
		{
			volumesApi=novaApi.getVolumeExtensionForZone("siel").get();
			Set<? extends Volume> volumes = volumesApi.list().toImmutableSet();

			for(Volume vol : volumes)
			{
				if(vol.getId().equals(volumeId))
				{
					if((vol.getStatus()).equals(Volume.Status.IN_USE))
						flag=false;
					else
						try {
							Thread.sleep(5000);
	 ...
	 */
	public static boolean createVolume (ComputeServiceContext context, String name, String size) {
		Optional<? extends VolumeApi> volumeTypeOption;
		...
		System.out.println("check 1");
		System.out.println("check 2");   
		Volume volume = volumesApi.create(Integer.parseInt(size), CreateVolumeOptions.Builder.name(name).description("description of test volume"));
	    System.out.println("check 3");
	  if(volume!=null) 
		  return true;
	  return false;
	 ...
	 * @param nodeId - Id of the node
	 */
	public static void deleteVolume (ComputeServiceContext context,String nodeId) {
		NovaApi novaApi = NovaApi.class.cast(context.getProviderSpecificContext() .getApi());
		VolumeApi volumesApi= novaApi.getVolumeExtensionForZone("siel").get();
		volumesApi.delete(nodeId);
		boolean flag=true;	
		int size=0;
		int i=0;
		while(flag)
	 ...
	 * @param nodeId - Id of the node
	 */
	public static void detachVolume (ComputeServiceContext context, String volumeId, String nodeId) {
		
		System.out.println("dettaching  " + volumeId);
		boolean flag=true;
		Map<Integer, Volume> volumesMap = new HashMap<Integer, Volume>();
		Optional<? extends VolumeApi> volumeTypeOption;
		NovaApi novaApi = NovaApi.class.cast(context.getProviderSpecificContext().getApi());
		...
		while(flag)
		{
			...
			for(Volume vol : volumes)
			{
				if(vol.getId().equals(volumeId))
				{
					if((vol.getStatus()).equals(Volume.Status.AVAILABLE))
						flag=false;
					else
						try {
							Thread.sleep(5000);
